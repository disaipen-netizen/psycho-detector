...
ВАЖНЕЙШЕЕ ПРАВИЛО — КОГО АНАЛИЗИРОВАТЬ:

1. Если это ПЕРЕПИСКА (двое или больше участников ведут диалог):
   - Анализируй того с кем общается пользователь (собеседника), не самого пользователя
   - Используй контекст и инструкции

2. Если это РАССКАЗ ПОЛЬЗОВАТЕЛЯ от первого лица (голосовое или текст где пользователь описывает поведение кого-то другого):
   - НЕ анализируй рассказчика! Он/она — пользователь, который хочет понять другого человека
   - Анализируй того О КОМ говорит пользователь — поведение, слова, паттерны описываемого человека
   - Пример: "мой парень постоянно меня критикует" → анализируй парня, не пользователя
   - Пример: "подруга меня игнорирует" → анализируй подругу

3. Анализируй ТОЛЬКО одного человека за раз, не несколько.
...

{
  ...
  "ideal_response": "Один из возможных вариантов ответа",
  "summary": "Итоговый вывод 2-3 предложения",
  "advice": {
    "title": "Общие рекомендации",
    "short": "Ключевая рекомендация — общая информация о взаимодействии с этим типом",
    "tactics": ["Совет 1", "Совет 2", "Совет 3"],
    "warning": "Чего лучше избегать"
  }
}`;
...
Проведи полный психологический анализ и верни JSON.`,
      ...
      // Определяем тип контента: переписка или рассказ пользователя о ком-то
      const isUserNarrative = hasAudios || (text && !text.includes(":") && !text.match(/\n.*\n/));

      const prefix = hasAudios
        ? `Это РАССКАЗ ПОЛЬЗОВАТЕЛЯ о другом человеке (или нескольких людях) — расшифровка ${audios.length > 1 ? `${audios.length} голосовых сообщений` : "голосового сообщения"}. Пользователь сам говорит, описывая поведение, слова и действия других людей в его жизни.`
        : "Текст для анализа. Это может быть либо переписка (несколько участников), либо рассказ пользователя о ком-то (от первого лица).";

      const narrativeRule = `
ВАЖНОЕ ПРАВИЛО ДЛЯ РАССКАЗА ПОЛЬЗОВАТЕЛЯ:
Если контент — это РАССКАЗ от первого лица о другом человеке (например, "мой парень мне сказал...", "она меня игнорирует...", "он постоянно..."), то:
- НЕ анализируй того кто рассказывает (пользователя/автора рассказа)
- Анализируй того О КОМ рассказывают — поведение, слова, паттерны другого человека описанные в рассказе
- Имя в "evidence" должно быть того человека о ком идёт речь
- Если упоминается несколько человек — выбери того о ком больше всего информации
- Если конкретного имени нет — используй "Описываемый человек" или роль ("Партнёр", "Подруга", "Коллега")
`;
